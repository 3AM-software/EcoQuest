    ...
    struct DeveloperToolbarView: View {
        ...
        @State private var resetQuestsScale: CGFloat = 1
                    ...
                    VStack(spacing: 16) {
                        ...
                        */
                        DevMenuButton(
                            ...
                            color: Color.cyan,
                            buttonScale: $resetQuestsScale,
                            action: {
                                triggerHaptic(.medium)
                                withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) {
                                    resetQuestsScale = 0.95
                                }
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                                    withAnimation(.spring(response: 0.3, dampingFraction: 0.6)) {
                                        resetQuestsScale = 1
    ...
    class AppWideState: ObservableObject {
        ...
        @Published var shouldRefreshMainApp = false
    }
    ...
    struct ContentView: View {
        ...
        @State private var mainAppID = UUID()
        ...
        var body: some View {
            Group {
                if globalState.isLoggingOut || isLoading {
                    LoadingScreen()
                } else {
                    if authViewModel.isLoggedIn {
                        ...
                        .onChange(of: appWideState.shouldRefreshMainApp) { oldValue, newValue in
                            guard newValue else { return }   // only react when it becomes `true`
                            mainAppID = UUID()
                            appWideState.shouldRefreshMainApp = false
                        }
...
struct SettingsView: View {
    ...
    @ObservedObject var authViewModel: UserAuthViewModel
    @StateObject private var currentauthViewModel = UserAuthViewModel()
                ...
                ZStack {
                    ...
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            VStack {
                                ...
                                HStack {
                                    Text("Display Name")
                                        ...
                                        .foregroundColor(.gray)
                                    Spacer()
                                    Text("\(currentauthViewModel.displayName)")
                                        .font(.custom("Fredoka", size: 16))
                                        .foregroundColor(ThemeColors.Content.primary(isDarkMode))
                                }
                                .padding(.horizontal)
                                ...
                                HStack {
                                    Text("Email")
                                        ...
                                        .foregroundColor(.gray)
                                    Spacer()
                                    Text(currentauthViewModel.email ?? "Not available")
                                        .font(.custom("Fredoka", size: 16))
                                        .foregroundColor(ThemeColors.Content.primary(isDarkMode))
                                }
                                .padding(.horizontal)
                            ...
                            Button(action: {
                                GlobalState.shared.isLoggingOut = true
                                DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                                    authViewModel.logOut()
                                    GlobalState.shared.isLoggingOut = false
                                }
                            }) {
                                HStack {
